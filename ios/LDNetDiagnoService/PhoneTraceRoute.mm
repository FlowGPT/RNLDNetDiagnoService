...
@property (nonatomic,strong) NSMutableString  *traceTTLLog;
...
//    timeout.tv_usec = 1000*kIcmpPacketTimeoutTime;
    ...
    if (res < 0) {
      [_traceTTLLog appendFormat:@"tracert %s , set timeout error..\n", [host UTF8String]];
       // log4cplus_warn("tracert %s , set timeout error..\n", [host UTF8String]);
    }
...
//       log4cplus_warn("access %s DNS error , remove this ip..\n",[host UTF8String]);
      [_traceTTLLog appendFormat:@"PhoneNetTracert access %s DNS error , remove this ip..\n",[host UTF8String]];
    }
...
{
  _traceTTLLog = [[NSMutableString alloc] initWithCapacity:20];
    if (![self verificationHost:host]) {
        self.isStopTracert = YES;
//         log4cplus_warn("there is no valid domain in the domain list , traceroute complete..\n");
      [_traceTTLLog appendFormat:@"PhoneNetTracert there is no valid domain in the domain list , traceroute complete..\n"];
        return;
    }
...
//    log4cplus_debug("begin tracert ip: %s \n", [self.host UTF8String]);
  [_traceTTLLog appendFormat:@"PhoneNetTracert begin tracert ip: %s \n", [self.host UTF8String]];
    do {
        int setTtlRes = setsockopt(socketClient,
                                   ...
                                   &ttl, sizeof(ttl));
        if (setTtlRes < 0) {
//            log4cplus_debug("set TTL for icmp packet error..\n");
          [_traceTTLLog appendFormat:@"PhoneNetTracert set TTL for icmp packet error..\n"];
        }
        ...
        for (int trytime = 0; trytime < kTracertSendIcmpPacketTimes; trytime++) {
            ...
            if ((int)sent < 0) {
              //  log4cplus_debug("send icmp packet failed, error info :%s\n", strerror(errno));
              [_traceTTLLog appendFormat:@"PhoneNetTracert send icmp packet failed, error info :%s\n", strerror(errno)];
                break;
            }
...
//            NSLog(@"rec:%d , last rec:%d",(int)rec,(int)self.lastRecTracertIcmpType);
            if (rec == PNetRecTracertIcmpType_noReply && self.lastRecTracertIcmpType == PNetRecTracertIcmpType_noReply) {
                ...
                if (continuousLossPacketRoute == kTracertRouteCount_noRes * kTracertSendIcmpPacketTimes) {
                    //log4cplus_debug("%d consecutive routes are not responding ,and end the tracert ip: %s\n", kTracertRouteCount_noRes, [self.host UTF8String]);
                  [_traceTTLLog appendFormat:@"%d consecutive routes are not responding ,and end the tracert ip: %s\n", kTracertRouteCount_noRes, [self.host UTF8String]];
...
//        log4cplus_debug("done tracert , ip :%s \n", [self.host UTF8String]);
      [_traceTTLLog appendFormat:@"done tracert , ip :%s \n", [self.host UTF8String]];
